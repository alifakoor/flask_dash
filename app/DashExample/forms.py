...
class AddDataForm(FlaskForm):
    ...
    ammonia = IntegerField('Ammonia', id='ammonia', validators=[
        InputRequired(), NumberRange(min=0, max=2000, message='Ammonia must be between 0 to 2000')
    ])
    ...
    oxygen = IntegerField('Oxygen', id='oxygen', validators=[
        InputRequired(), NumberRange(min=0, max=100, message='Oxygen must be between 0 to 100')
    ])
